...
These methods labels as anomaly the sequences that are far away from every other subsequence of the time series, for a given metric (euclidian, DTW). 
However, as explained in the article, one can be looking for anomalies that are not unique, for instance a sequence of anormal heartbeats.
...
\begin{enumerate}
    \item Extracting subsequences of length $3 \times l$  (randomly or with motif selection)
    \item Hierarchical clustering of the subsequences.
    \item Selecting the cluster $c$ in $\mathbb{C}$ (the set of clusters) that maximises the following quantity : $N(c) = \frac{frequency (c)^2 \times coverage (c) }{ \Sigma_{x \in \mathbb{C} } dist(center(c), center(x))}$\\
    With $frequency(c)$ the number of sequences in the cluster, $coverage(c)$ the time lag between the first and the last sequence in the cluster, and $center(c)$ the barycenter of the cluster.
    This quantity aims to select the cluster with the most sequences, and that is close to the other clusters. 
...
The authors describe several metric to label a subsequence as anormal. 
...
The key point of the method is that instead of comparing an anomaly to all the subsequences of the time-series and then comapring it to a very large number of identical pattern, the clustring steps enables to drastically reduce the number of comparisons. 
